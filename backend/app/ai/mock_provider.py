...
class DeterministicMockAIProvider(BaseAIProvider):
    ...
    async def analyze_document(self, text: str, title: str) -> Dict[str, Any]:
        ...
        is_lease = any(k in text_lower for k in ["lease", "rent", "lessor", "lessee", "tenant", "landlord", "demised premises"])
        is_employment = any(k in text_lower for k in ["employment", "employee", "employer", "salary", "probation", "resignation"])
        ...
        for c in raw_clauses:
            ...
            if any(k in c_lower for k in ["terminate", "termination", "notice period"]):
                ...
                if "unilateral" in c_lower or "7 days" in c_lower or "no right to terminate" in c_lower:
                    risk_level = "SEVERE"
                    ...
                    recommendation = "Demand equal notice period (minimum 30 days) for both parties."
                    risks.append({
                ...
                if "90 days" in c_lower or "forfeit" in c_lower or "5 months" in c_lower:
                    ...
                    plain_hi = "2 महीने के किराए के बराबर प्रतिदेय सुरक्षा जमा, जो 14 दिनों के भीतर वापस की जाएगी।"

            elif any(k in c_lower for k in ["rent", "charges", "interest", "penalty", "escalation"]):
            ...
            elif any(k in c_lower for k in ["non-refundable", "limitation of liability", "waives all rights"]):
        ...
        missing_clauses = []
        if is_lease:
            if not any("water" in c["text"].lower() or "structural" in c["text"].lower() for c in raw_clauses):
            ...
            if not any("pf" in c["text"].lower() or "provident" in c["text"].lower() for c in raw_clauses):
                missing_clauses.append({
    ...
    async def answer_question(self, question: str, chunks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        stop_words = {"what", "is", "the", "for", "in", "this", "how", "many", "does", "are", "and", "or", "of", "to", "a", "an"}
        q_tokens = [w.lower() for w in re.findall(r"\w+", question) if len(w) > 2 and w.lower() not in stop_words]
        ...
        for chunk in chunks:
            ...
            if score > best_score:
                ...
                sentence_scores = [(fuzz.token_set_ratio(question.lower(), s.lower()), s) for s in sentences if len(s) > 15]
                if sentence_scores:
    ...
    async def verify_claim(self, claim: str, chunks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if statutory_matches:
            ...
            is_contradicting = any(neg in claim_lower for neg in ["valid", "enforceable", "mandatory"]) and "void" in match.description.lower()
        ...
        for chunk in chunks:
            ...
            if set_ratio > best_set_ratio:
                ...
                best_chunk = chunk
                sentences = re.split(r"(?<=[.!?])\s+", content)
                sentence_scores = [(fuzz.token_set_ratio(claim_lower, s.lower()), s) for s in sentences if len(s) > 10]
                if sentence_scores:
                    sentence_scores.sort(key=lambda x: x[0], reverse=True)
                    best_quote = sentence_scores[0][1]
        ...
        if best_set_ratio >= 75:
            ...
            has_conflict_terms = any(t in claim_lower for t in ["not allowed", "never", "cannot terminate", "locked"])
            if has_conflict_terms and best_quote and any(t in best_quote.lower() for t in ["may terminate", "either party", "notice"]):
                status = "CONFLICTING"
                confidence = 0.85
        ...
        return {
            "claim_id": "CLM-DOC",
            "claim_text": claim,
            "evidence_source": f"Uploaded Document (Page {best_chunk.get('page_number', 1)})" if best_chunk else "Document",
    ...
    async def compare_documents(
        ...
        if "18%" in target_lower and "18%" not in base_lower:
            ...
            warnings.append("Target agreement increases late fee interest to 18% p.a. with daily compounding.")
        ...
        if "7 days" in target_lower and "no right to terminate" in target_lower:
            ...
            warnings.append("Termination rights altered to be completely asymmetric in favor of landlord.")
        ...
        verdict = "TARGET_MORE_HARSH" if target_high > base_high else ("BALANCED" if target_high == base_high else "TARGET_MORE_FAVORABLE")
    ...
    async def interpret_clause(
        ...
        for s in sentences:
            ...
            if any(w in s_lower for w in ["may", "has the right", "is entitled to", "at its sole option", "option to"]):
                rights.append(s.strip())
            if any(w in s_lower for w in ["shall not", "must not", "prohibited", "neither party shall", "will not", "no right"]):
            ...
            if any(w in s_lower for w in ["within", "prior to", "before", "by the", "on or before", "grace period"]):
                deadlines.append(s.strip())
            if any(w in s_lower for w in ["penalty", "forfeit", "interest of", "liquidated damages", "late charge", "fine"]):
                penalties.append(s.strip())
            if any(w in s_lower for w in ["terminate", "termination", "cancel", "vacate", "eviction", "expire"]):
                termination_conditions.append(s.strip())

        parties = []
        party_keywords = ["landlord", "tenant", "employer", "employee", "service provider", "client", "lender", "borrower", "disclosing party", "receiving party", "licensor", "licensee"]
        ...
        monetary_values = [f"{c['currency']} {c['amount']:,.0f}" for c in deterministic_facts.get("currencies", [])]

        jurisdiction = None
        jur_m = re.search(r'(?:courts\s+(?:of|at|in))\s+([A-Z][a-zA-Z]+)', clause_text, re.IGNORECASE)
        if jur_m:
        ...
        if "laws of india" in lower or "law of india" in lower:
            ...
            gov_m = re.search(r'(?:governed\s+by\s+(?:the\s+)?laws\s+of)\s+([A-Z][a-zA-Z]+)', clause_text, re.IGNORECASE)
            if gov_m:
