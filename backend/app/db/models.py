...
class User(Base):
    ...
    role = Column(String(50), default="user", nullable=False)  # 'user', 'advocate', 'admin', 'USER', 'ADMIN', 'SYSTEM_OPERATOR'
...
class Membership(Base):
    ...
    role = Column(String(50), default="member", nullable=False)  # 'owner', 'admin', 'advocate', 'member', 'viewer'
    created_at = Column(DateTime(timezone=True), default=utc_now, nullable=False)
    updated_at = Column(DateTime(timezone=True), default=utc_now, onupdate=utc_now, nullable=False)

    __table_args__ = (
        UniqueConstraint("organization_id", "user_id", name="uq_membership_org_user"),
        CheckConstraint("role IN ('owner', 'admin', 'advocate', 'member', 'viewer')", name="ck_membership_role"),
    )
...
class Document(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("file_type IN ('pdf', 'docx', 'txt', 'png', 'jpg', 'jpeg')", name="ck_document_file_type"),
    ...
    risk_findings = relationship("RiskFinding", back_populates="document", cascade="all, delete-orphan")
    ...
    briefs = relationship("ConsultationBrief", back_populates="document", cascade="all, delete-orphan")
...
class DocumentPage(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    document_id = Column(Integer, ForeignKey("documents.id", ondelete="CASCADE"), nullable=False, index=True)
    version_id = Column(Integer, ForeignKey("document_versions.id", ondelete="CASCADE"), nullable=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint("ocr_confidence >= 0.0 AND ocr_confidence <= 1.0", name="ck_ocr_confidence_range"),
    )

    document = relationship("Document", back_populates="pages")
    citations = relationship("Citation", back_populates="document_page", cascade="all, delete-orphan")
...
class Clause(Base):
    ...
    title = Column(String(255), nullable=False)
    category = Column(String(100), nullable=False)  # Termination, Rent Escalation, Non-Compete, etc.
    raw_text = Column(Text, nullable=False)
    clean_text = Column(Text, nullable=False)
    ...
    __table_args__ = (
        CheckConstraint("risk_level IN ('LOW', 'MEDIUM', 'HIGH', 'SEVERE', 'CRITICAL')", name="ck_clause_risk_level"),
    )
...
class ClauseRelationship(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    source_clause_id = Column(Integer, ForeignKey("clauses.id", ondelete="CASCADE"), nullable=False, index=True)
    target_clause_id = Column(Integer, ForeignKey("clauses.id", ondelete="CASCADE"), nullable=False, index=True)
    relationship_type = Column(String(50), nullable=False)  # 'SUPERSEDES', 'CONFLICTS_WITH', 'DEPENDS_ON', 'MODIFIES'
...
class LegalSource(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    uuid = Column(String(36), unique=True, index=True, default=generate_uuid, nullable=False)
    code = Column(String(100), unique=True, index=True, nullable=False)  # e.g., 'BNS_2023', 'CPA_2019'
    title = Column(String(255), nullable=False)
    short_name = Column(String(100), nullable=False)
    jurisdiction = Column(String(100), default="Union of India", nullable=False)
    authority_level = Column(String(50), nullable=False)  # 'PARLIAMENT_ACT', 'STATE_ACT', 'SUPREME_COURT_RULING', 'REGULATORY_RULE'
    publication_date = Column(DateTime(timezone=True), nullable=True)
    effective_from = Column(DateTime(timezone=True), nullable=False)
    effective_to = Column(DateTime(timezone=True), nullable=True)
    status = Column(String(50), default="ACTIVE", nullable=False)  # 'DRAFT', 'ACTIVE', 'AMENDED', 'REPEALED', 'SUPERSEDED'
    ...
    __table_args__ = (
        CheckConstraint("effective_to IS NULL OR effective_to >= effective_from", name="ck_legal_source_effective_dates"),
        CheckConstraint("status IN ('DRAFT', 'ACTIVE', 'AMENDED', 'REPEALED', 'SUPERSEDED')", name="ck_legal_source_status"),
        CheckConstraint(
    ...
    versions = relationship("LegalSourceVersion", back_populates="legal_source", cascade="all, delete-orphan")
    source_documents = relationship("SourceDocument", back_populates="legal_source", cascade="all, delete-orphan")
...
class LegalSourceVersion(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    legal_source_id = Column(Integer, ForeignKey("legal_sources.id", ondelete="CASCADE"), nullable=False, index=True)
...
class SourceDocument(Base):
    ...
    citations = relationship("Citation", back_populates="source_document", cascade="all, delete-orphan")
...
class Citation(Base):
    ...
    document_page_id = Column(Integer, ForeignKey("document_pages.id", ondelete="CASCADE"), nullable=True, index=True)
    ...
    __table_args__ = (
        CheckConstraint("source_type IN ('STATUTE', 'DOCUMENT_PAGE', 'COURT_CASE')", name="ck_citation_source_type"),
        CheckConstraint("confidence_score >= 0.0 AND confidence_score <= 1.0", name="ck_citation_confidence_score"),
    )
...
class EvidenceItem(Base):
    ...
    citation_id = Column(Integer, ForeignKey("citations.id", ondelete="SET NULL"), nullable=True, index=True)
...
class Claim(Base):
    ...
    category = Column(String(50), default="CONTRACTUAL", nullable=False)  # 'FACTUAL', 'STATUTORY', 'CONTRACTUAL'
    created_at = Column(DateTime(timezone=True), default=utc_now, nullable=False)

    __table_args__ = (
        CheckConstraint("category IN ('FACTUAL', 'STATUTORY', 'CONTRACTUAL')", name="ck_claim_category"),
    )
...
class ClaimVerification(Base):
    ...
    evidence_item_id = Column(Integer, ForeignKey("evidence_items.id", ondelete="SET NULL"), nullable=True, index=True)
    status = Column(String(50), nullable=False)  # 'SUPPORTED', 'PARTIALLY_SUPPORTED', 'UNSUPPORTED', 'CONFLICTING', 'UNVERIFIED'
    confidence_score = Column(Float, default=0.0, nullable=False)
    reasoning = Column(Text, nullable=False)
    verified_by = Column(String(50), default="AI_MODEL", nullable=False)  # 'AI_MODEL', 'HUMAN_ADVOCATE', 'STATUTORY_RULE'
    created_at = Column(DateTime(timezone=True), default=utc_now, nullable=False)
    ...
    __table_args__ = (
        ...
        CheckConstraint("confidence_score >= 0.0 AND confidence_score <= 1.0", name="ck_claim_ver_confidence"),
        CheckConstraint("verified_by IN ('AI_MODEL', 'HUMAN_ADVOCATE', 'STATUTORY_RULE')", name="ck_claim_ver_by"),
    )
...
class RiskFinding(Base):
    ...
    __table_args__ = (
        CheckConstraint("severity IN ('LOW', 'MEDIUM', 'HIGH', 'SEVERE', 'CRITICAL')", name="ck_risk_finding_severity"),
    )
...
class Comparison(Base):
    ...
    uuid = Column(String(36), unique=True, index=True, default=generate_uuid, nullable=False)
    user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    base_document_id = Column(Integer, ForeignKey("documents.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    __table_args__ = (
        CheckConstraint("base_document_id != target_document_id", name="ck_comparison_distinct_docs"),
        CheckConstraint(
    ...
    findings = relationship("ComparisonFinding", back_populates="comparison", cascade="all, delete-orphan")
...
class ComparisonFinding(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    comparison_id = Column(Integer, ForeignKey("comparisons.id", ondelete="CASCADE"), nullable=False, index=True)
    base_clause_id = Column(Integer, ForeignKey("clauses.id", ondelete="SET NULL"), nullable=True, index=True)
    target_clause_id = Column(Integer, ForeignKey("clauses.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    __table_args__ = (
        CheckConstraint("change_type IN ('ADDED', 'REMOVED', 'MODIFIED', 'UNCHANGED')", name="ck_comp_finding_change_type"),
    )
...
class ComparisonResult(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    base_document_id = Column(Integer, ForeignKey("documents.id", ondelete="CASCADE"), nullable=False)
...
class LegalAidResource(Base):
    ...
    uuid = Column(String(36), unique=True, index=True, default=generate_uuid, nullable=False)
    name = Column(String(255), nullable=False)
    organization_type = Column(String(50), nullable=False)  # 'NALSA', 'SLSA', 'DLSA', 'TLSC', 'TELE_LAW', 'CONSUMER_FORUM', 'CYBER_HELPLINE'
...
class Consent(Base):
    ...
    uuid = Column(String(36), unique=True, index=True, default=generate_uuid, nullable=False)
    user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    consent_type = Column(String(50), nullable=False)  # 'PII_REDACTION', 'EDUCATIONAL_DISCLAIMER_ACK', 'TERMS_OF_SERVICE', 'DATA_PROCESSING'
...
class ProcessingJob(Base):
    ...
    uuid = Column(String(36), unique=True, index=True, default=generate_uuid, nullable=False)
    document_id = Column(Integer, ForeignKey("documents.id", ondelete="CASCADE"), nullable=False, index=True)
    job_type = Column(String(50), nullable=False)  # 'OCR_EXTRACTION', 'PII_SANITIZATION', 'VECTOR_INDEXING', 'DEEP_ANALYSIS'
...
class AICacheEntry(Base):
    ...
    __table_args__ = (
        Index("ix_aicache_key_ver", "cache_key", "source_registry_version"),
    )
