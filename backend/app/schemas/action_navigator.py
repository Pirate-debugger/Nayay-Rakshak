...
class FactItem(BaseModel):
    ...
    fact: str = Field(
        ..., min_length=10, description="An objectively verifiable fact extracted from the document"
    )
    source: str = Field(
...
class ImportantDateItem(BaseModel):
    ...
    consequence: str = Field(..., description="What may happen if this date is missed or ignored")
...
class QuestionItem(BaseModel):
    model_config = ConfigDict(extra="forbid")
    question: str = Field(
        ..., min_length=10, description="A specific question the user should consider asking"
    )
    purpose: str = Field(..., description="Why this question is important to have answered")
    ask_whom: str = Field(
        ...,
...
class ActionStep(BaseModel):
    ...
    step_number: int = Field(
        ..., ge=0, description="Execution order (1 = first; 0 = placeholder during assembly)"
    )
    action: str = Field(
        ...,
        min_length=10,
        description="The specific, concrete action to take. MUST NOT guarantee a legal outcome.",
    )
    reason: str = Field(
    ...
    dependency: Optional[str] = Field(
        None, description="Step or action that should be completed before this one (if any)"
    )
...
class EscalationTrigger(BaseModel):
    ...
    reason: str = Field(..., description="Plain-language explanation of why this trigger fired")
...
class ActionNavigatorResponse(BaseModel):
    ...
    important_documents: List[ImportantDocumentItem] = Field(
        default_factory=list, description="Documents the user should collect, verify, or keep safe"
    )
    ...
    professional_review_urgency: UrgencyLevel = Field(
        UrgencyLevel.LOW, description="Urgency of professional review if recommended"
    )
