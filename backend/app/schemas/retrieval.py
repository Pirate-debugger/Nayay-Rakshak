...
class RetrievalFilter(BaseModel):
    ...
    jurisdiction: Optional[str] = Field(
        None,
        description="Target jurisdiction (e.g. 'Union of India', 'NCT of Delhi', 'Maharashtra')",
    )
    allow_cross_jurisdiction: bool = Field(
        False, description="Whether to allow mixing different state jurisdictions. Default FALSE."
    )
    court: Optional[str] = Field(
        None,
        description="Specific court filter (e.g. 'Supreme Court of India', 'Delhi High Court')",
    )
    legal_domain: Optional[str] = Field(
        None,
        description="Domain filter (e.g. 'criminal', 'tenancy', 'contract', 'consumer', 'privacy')",
    )
    as_of_date: Optional[datetime] = Field(
        None, description="Temporal reference date for filtering active legal validity"
    )
    min_authority_tier: Optional[SourceTier] = Field(
        None, description="Minimum acceptable authority tier"
    )
...
class CitationRecord(BaseModel):
    model_config = ConfigDict(extra="forbid")
    source_id: str = Field(
        ..., description="Canonical source ID (e.g. BNS-2023-SEC-318, ICA-1872-SEC-27)"
    )
    title: str = Field(..., min_length=3, description="Official title of the statute or document")
    authority: str = Field(
        ..., description="Originating authority, e.g. Parliament of India / Supreme Court of India"
    )
    authority_tier: SourceTier = Field(..., description="Source hierarchy tier: 1 to 4")
    evidence_type: EvidenceType = Field(
        ..., description="USER_DOCUMENT_EVIDENCE vs LEGAL_AUTHORITY vs SECONDARY_INFORMATION"
    )
    section_or_page: str = Field(
        ..., description="Specific statutory section or user document page reference"
    )
    publication_date: Optional[str] = Field(None, description="Official Gazette publication date")
    effective_date: Optional[str] = Field(
        None, description="Date from which the provision is enforceable"
    )
    ...
    url_or_reference: str = Field(
        ..., description="Authentic official Gazette or India Code URL/citation. NEVER invented."
    )
    supporting_text: str = Field(
        ..., min_length=5, description="Exact verbatim statutory text or document quote"
    )
    relevance_score: float = Field(
        ..., ge=0.0, le=1.0, description="Relevance score from retriever/reranker"
    )
    provenance: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata trace (hash, paragraph offset, database ID)"
    )
...
class RetrievalResponse(BaseModel):
    ...
    explicit_unretrieved_disclaimer: Optional[str] = Field(
        None, description="Explicit notice when authoritative evidence cannot be retrieved"
    )
...
class RetrievalEvaluationRequest(BaseModel):
    ...
    test_queries: List[Dict[str, Any]] = Field(
        ..., min_length=1, description="List of benchmark query specs"
    )
    k_values: List[int] = Field(
        default=[1, 3, 5], description="Cutoff thresholds for Precision and NDCG"
    )
