...
class QAClassifier:
    ...
    def classify_and_extract_context(
        ...
        domain = "general"
        if any(k in q_lower for k in ["rent", "lease", "tenant", "landlord", "eviction", "security deposit", "premises"]):
            domain = "tenancy"
        elif any(k in q_lower for k in ["employee", "employer", "salary", "resignation", "non-compete", "notice period", "probation"]):
            domain = "employment"
        elif any(k in q_lower for k in ["cheating", "theft", "fir", "police", "crime", "bns", "ipc", "criminal", "bail"]):
            domain = "criminal"
        elif any(k in q_lower for k in ["consumer", "defective", "refund", "unfair trade", "e-daakhil"]):
            domain = "consumer"
        ...
        is_doc_focused = any(k in q_lower for k in [
            "my agreement", "my contract", "my lease", "this clause", "clause", "page",
            "what does the document say", "in my agreement", "my notice period", "my deposit"
        ])
        is_statute_focused = any(k in q_lower for k in [
            "indian law", "law in india", "under law", "statute", "section", "act", "bns", "ipc",
            "contract act", "supreme court", "high court", "legal right", "legal", "valid",
            "enforceable", "legally valid", "penalty", "punishment", "allowed under law", "court"
        ])
        ...
        if has_doc and is_doc_focused and is_statute_focused:
            ...
            req_type = QuestionRequirementType.BOTH if any(k in q_lower for k in ["valid", "allowed", "legal", "can they"]) else QuestionRequirementType.DOCUMENT_EVIDENCE
        else:
    ...
    def _identify_missing_facts(
        ...
        if domain == "tenancy":
            if not any(k in q_lower for k in ["delhi", "mumbai", "maharashtra", "bangalore", "karnataka", "chennai"]):
                missing.append("Specific state or union territory where the rented property is physically situated (state rent control acts differ significantly).")
            if not any(k in q_lower for k in ["written", "verbal", "registered"]):
                missing.append("Whether the tenancy agreement is officially registered or an unregistered/verbal understanding.")
            if "deposit" in q_lower and not any(k in q_lower for k in ["month", "amount", "₹", "rs"]):
                missing.append("The exact quantum of security deposit and monthly rent amount.")

        elif domain == "employment":
            if "notice" in q_lower and not any(k in q_lower for k in ["probation", "confirmed", "permanent"]):
                missing.append("Whether the employee is currently in their probation period or has been confirmed as permanent staff.")
            if "non-compete" in q_lower and not any(k in q_lower for k in ["during", "after", "post"]):
                missing.append("Whether the non-compete restriction applies during active employment or post-cessation (post-service covenants face strict unenforceability under Section 27).")

        elif domain == "criminal":
            if not any(k in q_lower for k in ["before", "after", "2024", "2023", "bns", "ipc"]):
                missing.append("The exact date when the alleged offence occurred (determines whether legacy IPC 1860 or Bharatiya Nyaya Sanhita 2023 applies).")
            if not any(k in q_lower for k in ["fir", "complaint", "police"]):
                missing.append("Whether an FIR, e-FIR, or formal police complaint has already been registered.")

        elif domain == "consumer":
            if not any(k in q_lower for k in ["notice", "written", "complaint"]):
                missing.append("Whether a formal written legal notice has been served upon the trader/service provider giving reasonable cure time.")

        if not has_doc and any(k in q_lower for k in ["my clause", "my contract", "valid", "can my landlord"]):
            missing.append("The exact signed text of the agreement is not available; analysis is based solely on general legal principles.")
