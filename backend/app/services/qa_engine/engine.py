...
class LegalQAEngine:
    ...
    def __init__(self, classifier=None, retriever=None, hedging_gate=None, localizer=None):
    ...
    async def answer_legal_question(
        self, req: QARequest, document_chunks: Optional[List[Dict[str, Any]]] = None
    ) -> QAResponse:
        """
        ...
        if is_inj:
            raise PromptInjectionDetected(
                f"Security Alert: Suspicious prompt pattern detected in question: '{pattern}'"
            )
        ...
        retrieval_filters = RetrievalFilter(
            ...
            legal_domain=context_meta.legal_domain
            if context_meta.legal_domain != "general"
            else None,
        )
        ...
        formatted_answer = self._format_markdown_answer(
            sections, target_lang=target_lang, localized_data=localized_data
        )
    ...
    def _synthesize_sections(
        ...
        if has_uploaded_doc and not is_found_in_doc:
            plain_answer = (
                "I could not verify this from the available sources in the uploaded document."
            )
            conf_score = 0.0
        ...
        uncertainty_parts.append(
            "- Judicial determination: Contractual terms may be interpreted differently depending on specific evidentiary context and regional High Court rulings."
        )
