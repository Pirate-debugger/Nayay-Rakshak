...
class QAHedgingGate:
    ...
    FORBIDDEN_OUTCOME_PATTERNS = [
        (
            r"(?i)\byou\s+will\s+(?:definitely\s+)?win\b",
            "your position may find statutory support, though outcomes depend on judicial determination",
        ),
        (
            r"(?i)\bthis\s+is\s+definitely\s+legal\b",
            "this appears consistent with customary statutory principles based on current information",
        ),
        (
            r"(?i)\bthis\s+is\s+definitely\s+(?:illegal|unlawful)\b",
            "this appears problematic under governing Indian statutory provisions",
        ),
        (
            r"(?i)\byour\s+case\s+will\s+succeed\b",
            "your matter presents tenable legal arguments, subject to evidence and court discretion",
        ),
        (
            r"(?i)\b(?:guaranteed|certain)\s+to\s+(?:win|succeed)\b",
            "strongly arguable, but never guaranteed in legal proceedings",
        ),
        (
            r"(?i)\bthis\s+clause\s+is\s+(?:completely\s+)?void\b",
            "this clause appears vulnerable to challenge under statutory grounds",
        ),
        (
            r"(?i)\byou\s+have\s+no\s+liability\b",
            "your liability appears limited based on the available terms",
        ),
        (
            r"(?i)\b100%\s*(?:legal|binding|winnable)\b",
            "substantially supported by available documentation",
        ),
    ]
    ...
    def enforce_hedging(self, text: str) -> Tuple[str, bool]:
        ...
        if not any(
            ...
            if (
                len(hedged.strip()) > 0
                and not hedged.startswith("Note:")
                and not hedged.startswith("Based on")
            ):
