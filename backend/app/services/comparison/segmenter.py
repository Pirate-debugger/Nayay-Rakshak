...
def segment_document_for_comparison(
    ...
    if chunks and len(chunks) > 0:
        ...
        for idx, chunk in enumerate(chunks):
            if isinstance(chunk, dict):
                ...
                content = getattr(chunk, "clean_content", "") or getattr(chunk, "raw_content", "") or ""
            ...
            if heading == "General Terms" and content:
                ...
                m_head = re.match(r'^(?:Clause|Section|Article|\b[0-9]{1,2}[\.\)])\s*([^\n\:\.]{3,80})', first_line, re.IGNORECASE)
                if m_head:
            ...
            num_match = re.match(r'^(?:Clause|Section|Article|\b)?\s*([0-9]+(?:\.[0-9]+)*|[A-Z])[\.\:\s]', heading, re.IGNORECASE)
            sec_num = num_match.group(1) if num_match else str(idx + 1)
            
            clauses.append(ComparisonClause(
    ...
    if len(blocks) <= 1:
        ...
        for line in single_lines:
            ...
            if not s_line:
                continue
            m2 = re.match(r'^(?:Clause|Section|Article|\b[0-9]{1,2}[\.\)])\s*(.*)', s_line, re.IGNORECASE)
            if m2 and curr_p:
                b_txt = "\n".join(curr_p).strip()
                blocks.append({
                    "heading": curr_h,
                ...
                s_char += len(b_txt) + 1
                curr_p = [s_line]
                num_match = re.match(r'^(?:Clause|Section|Article|\b)?\s*([0-9]+(?:\.[0-9]+)*|[A-Z])[\.\:\s]', s_line, re.IGNORECASE)
    ...
    for idx, b in enumerate(blocks):
        result.append(ComparisonClause(
            clause_id=f"CLAUSE-{idx+1:02d}",
