...
class QueryClassifier:
    ...
    def classify(
        self, query: str, has_user_document: bool = False
    ) -> Tuple[QueryIntent, RetrievalFilter]:
        q_lower = query.lower()
        ...
        if any(
            ...
            k in q_lower
            for k in ["bns", "bnss", "bsa", "new law", "current law", "2024", "2025", "2026"]
        ):
        ...
        if any(
            k in q_lower
            for k in ["rent", "lease", "tenant", "landlord", "eviction", "security deposit"]
        ):
            legal_domain = "tenancy"
        elif any(
            k in q_lower
            for k in ["cheating", "theft", "crime", "fir", "police", "bns", "ipc", "fraud"]
        ):
            legal_domain = "criminal"
        elif any(
            k in q_lower
            for k in ["non-compete", "salary", "employment", "employee", "notice period"]
        ):
            legal_domain = "employment"
        elif any(
            k in q_lower for k in ["consumer", "defective", "unfair contract", "product liability"]
        ):
