...
class RiskEngine:
    ...
    async def analyze_document_risks(
        ...
        if ai_provider:
            try:
                ...
                for air in ai_risks:
                    if not any(
                        r.category == air.category and r.title == air.title for r in all_risks
                    ):
    ...
    def _is_clause_camouflaged(self, clause: StructuredClauseRecord) -> bool:
        ...
        if any(dh in sec for dh in deceptive_headings):
            ...
            if any(
                k in t_lower
                for k in ["indemn", "non-compete", "penalty", "waive", "liquidated damages"]
            ):
                return True
        return False

    def _detect_conflicting_clauses(
        self, clauses: List[StructuredClauseRecord]
    ) -> List[RiskRecord]:
        """
        ...
        for c in clauses:
            ...
            if any(
                ...
                for k in [
                    ...
                    "cannot vacate prior",
                    "liable for rent for the entire unexpired term",
                ]
            ):
            ...
            if quote_clean and quote_clean in document_text:
                ...
                ai_risks.append(
                    RiskRecord(
                        ...
                        plain_language_explanation=r.get(
                            "plain_language_explanation",
                            "This clause contains language worth clarifying before signing. Note: Legal validity cannot be established without judicial review.",
                        ),
                        why_it_matters=r.get(
                            "why_it_matters",
                            "May create operational friction or unexpected liability exposure.",
                        ),
                        evidence=EvidenceLocation(
                            page_number=1,
                        ...
                        confidence=0.88,
                        recommended_question=r.get(
                            "recommended_question",
                            "Can this clause be clarified with specific reciprocal definitions?",
                        ),
                        professional_review_recommended=sev
                        in [RiskSeverity.HIGH, RiskSeverity.CRITICAL],
                        rule_id=None,
                        rule_version=self.ruleset_version,
