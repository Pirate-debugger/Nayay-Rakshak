...
async def get_document_status(
    ...
    jobs_q = select(ProcessingJob).where(ProcessingJob.document_id == doc.id).order_by(ProcessingJob.created_at.desc())
...
async def load_sample_document(
    ...
    sample_files = {
        "standard_residential_lease_delhi": ("Standard Residential Lease (Delhi NCR)", "sample_documents/standard_residential_lease_delhi.txt"),
        "harsh_landlord_lease_delhi": ("Harsh Landlord Lease (Delhi NCR - High Risk)", "sample_documents/harsh_landlord_lease_delhi.txt"),
        "employment_agreement_tech_bangalore": ("Tech Employment & Non-Compete Agreement (Bangalore)", "sample_documents/employment_agreement_tech_bangalore.txt"),
        "consumer_services_contract": ("Consumer Subscription Terms (Unfair Terms)", "sample_documents/consumer_services_contract.txt"),
    }
    ...
    if not os.path.exists(rel_path):
        candidate_parent = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../..", rel_path))
        ...
        if os.path.exists(candidate_parent):
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Sample file {rel_path} not found on server.")
