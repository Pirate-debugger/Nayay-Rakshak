...
async def test_user_and_organization_isolation(test_db_session: AsyncSession):
    ...
    user_a = User(email="citizen.a@example.com", hashed_password="hashed_a", full_name="A Citizen", role="user")
    user_b = User(email="citizen.b@example.com", hashed_password="hashed_b", full_name="B Citizen", role="user")
...
async def test_database_check_and_unique_constraints(test_db_session: AsyncSession):
    ...
    user = User(email="test.constraints@example.com", hashed_password="hashed_pw", full_name="Constraint Tester")
    test_db_session.add(user)
    await test_db_session.commit()
    user_id = user.id

    # 1. Unique email constraint
    duplicate_user = User(email="test.constraints@example.com", hashed_password="other", full_name="Duplicate")
...
async def test_cascading_deletion_behavior(test_db_session: AsyncSession):
    ...
    user = User(email="cascade.test@example.com", hashed_password="hashed_pw", full_name="Cascade User")
    test_db_session.add(user)
    await test_db_session.commit()
    ...
    assert (await test_db_session.execute(select(DocumentVersion).where(DocumentVersion.document_id == doc_id))).scalar_one_or_none() is None
    assert (await test_db_session.execute(select(DocumentPage).where(DocumentPage.document_id == doc_id))).scalar_one_or_none() is None
    assert (await test_db_session.execute(select(DocumentSection).where(DocumentSection.document_id == doc_id))).scalar_one_or_none() is None
    assert (await test_db_session.execute(select(Clause).where(Clause.document_id == doc_id))).scalar_one_or_none() is None
    assert (await test_db_session.execute(select(RiskFinding).where(RiskFinding.document_id == doc_id))).scalar_one_or_none() is None
    assert (await test_db_session.execute(select(ProcessingJob).where(ProcessingJob.document_id == doc_id))).scalar_one_or_none() is None
    assert (await test_db_session.execute(select(ConsultationBrief).where(ConsultationBrief.document_id == doc_id))).scalar_one_or_none() is None
...
async def test_document_versioning_and_soft_delete(test_db_session: AsyncSession):
    ...
    user = User(email="versioning.test@example.com", hashed_password="hashed_pw", full_name="Version User")
    test_db_session.add(user)
    await test_db_session.commit()
    user_id = user.id
    ...
    v1 = DocumentVersion(document_id=doc_id, version_number=1, storage_path="./storage/v1.pdf", change_summary="Initial upload")
    v2 = DocumentVersion(document_id=doc_id, version_number=2, storage_path="./storage/v2.pdf", change_summary="Landlord counter-offer")
    test_db_session.add_all([v1, v2])
    await test_db_session.commit()

    # Verify versions are queryable in order
    q_vers = select(DocumentVersion).where(DocumentVersion.document_id == doc_id).order_by(DocumentVersion.version_number.desc())
    ...
    v_dup = DocumentVersion(document_id=doc_id, version_number=2, storage_path="./storage/v2_dup.pdf")
    ...
    doc = (await test_db_session.execute(select(Document).where(Document.id == doc_id))).scalar_one()
    ...
    all_docs = (await test_db_session.execute(select(Document).where(Document.user_id == user_id))).scalars().all()
...
async def test_database_seed_and_statutory_grounding(test_db_session: AsyncSession):
    ...
    bns = (await test_db_session.execute(select(LegalSource).where(LegalSource.code == "BNS_2023"))).scalar_one_or_none()
    assert bns is not None
    assert bns.authority_level == "PARLIAMENT_ACT"
    assert bns.status == "ACTIVE"

    # Verify Section 318 (Cheating)
    s318 = (await test_db_session.execute(
        select(SourceDocument).where(SourceDocument.legal_source_id == bns.id, SourceDocument.section_number == "Section 318")
    )).scalar_one_or_none()
    ...
    ica = (await test_db_session.execute(select(LegalSource).where(LegalSource.code == "ICA_1872"))).scalar_one_or_none()
    assert ica is not None
    s27 = (await test_db_session.execute(
        select(SourceDocument).where(SourceDocument.legal_source_id == ica.id, SourceDocument.section_number == "Section 27")
    )).scalar_one_or_none()
    ...
    demo_user = (await test_db_session.execute(select(User).where(User.email == "demo.citizen@nyayarakshak.in"))).scalar_one_or_none()
    assert demo_user is not None
    assert demo_user.is_demo is True
    assert "[DEMO]" in demo_user.full_name

    demo_org = (await test_db_session.execute(select(Organization).where(Organization.slug == "demo-citizen-clinic"))).scalar_one_or_none()
