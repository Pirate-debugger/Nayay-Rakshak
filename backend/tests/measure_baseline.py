...
from app.services.document_parser import chunk_document_text, normalize_text
...
async def run_benchmarks():
    ...
    sample_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../sample_documents/standard_residential_lease_delhi.txt"))
    ...
    t0 = time.perf_counter()
    for q in ["What is the lock-in period penalty?", "Can landlord enter without notice?", "How much security deposit?"]:
        res = await legal_retriever.retrieve(
            query=q,
            document_chunks=[{"content": c["content"], "page_number": c["page_number"]} for c in chunks],
            top_k=5
        )
    ...
    t0 = time.perf_counter()
    analysis = await ai.analyze_document(sanitized_text, "Standard Residential Lease")
    t_analysis = (time.perf_counter() - t0)
    results["ai_analysis_ms"] = t_analysis * 1000

    t0 = time.perf_counter()
    qa_ans = await ai.answer_question("What is the rent amount?", [{"clean_content": c["clean_content"], "page_number": c["page_number"]} for c in chunks])
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if not user:
                user = User(id=1, email="test.perf@example.com", hashed_password="pw", full_name="Perf Tester", role="USER")
                session.add(user)
        ...
        t0 = time.perf_counter()
        doc_resp = await client.post("/api/v1/documents/load-sample", data={"sample_key": "standard_residential_lease_delhi"}, headers=headers)
        ...
        t0 = time.perf_counter()
        analysis_resp = await client.post(f"/api/v1/analysis/{doc_id}", headers=headers)
        t_http_analysis = (time.perf_counter() - t0) * 1000
        results["http_analysis_ms"] = t_http_analysis

        # Measure Q&A Endpoint (Cold Query 1)
        t0 = time.perf_counter()
        qa_resp1 = await client.post("/api/v1/qa/", json={"document_id": doc_id, "question": "What is the security deposit amount?", "language": "en"}, headers=headers)
        ...
        t0 = time.perf_counter()
        qa_resp2 = await client.post("/api/v1/qa/", json={"document_id": doc_id, "question": "What is the security deposit amount?", "language": "en"}, headers=headers)
